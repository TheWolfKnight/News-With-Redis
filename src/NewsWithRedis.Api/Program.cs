  ...
  public class Program
  {
    public static void Main(string[] args)
    {
      ...
      { // Could replace Json here for more speed
        var cString = Environment.GetEnvironmentVariable("MSSQL_SQL_STRING", EnvironmentVariableTarget.Process)
          ?? builder.Configuration["cStrings:SQl:DefaultConnection"];

        Console.WriteLine($" [INFO: Infrastrucure::Program::Main] MsSql connection: {cString}");

        return new SqlClient(cString!);
      });
      builder.Services.AddSingleton<ConnectionMultiplexer>(y =>
      {
        var cString = Environment.GetEnvironmentVariable("REDIS_CONNECTION", EnvironmentVariableTarget.Process)
          ?? builder.Configuration["cStrings:RED:DefaultConnection"];

        Console.WriteLine($" [INFO: Infrastrucure::Program::Main] Redis connection: {cString}");
